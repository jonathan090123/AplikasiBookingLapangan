...
<form method="GET" action="{{ route('pesanan.index') }}" class="mb-6 grid grid-cols-1 md:grid-cols-3 gap-4">
    <input type="text" name="nama_pemesan" value="{{ request('nama_pemesan') }}" placeholder="Cari nama pemesan..." class="border rounded p-2">
    ...
    <tbody>
        @forelse ($pesanan as $index => $p)
            <tr class="hover:bg-gray-50">
                ...
                <td class="p-3 border">{{ $p->jadwal->jam_mulai }} - {{ $p->jadwal->jam_selesai }}</td>
                <td class="p-3 border">
                    <a href="{{ route('pesanan.edit', $p->id) }}" class="text-blue-600 hover:underline mr-3">Edit</a>
                    <form action="{{ route('pesanan.edit', $p->id) }}" method="POST" class="inline" onsubmit="return confirm('Yakin ingin menghapus?');">
                        @csrf
                        @method('DELETE')
